    ...
    <div className="max-w-5xl mx-auto w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-[#A1A1AA] mt-1">Observe model token expenditures, query latencies, and execution success rates.</p>
        </div>
        <select
          className="bg-slate-900/50 border border-[rgba(255,255,255,0.06)] rounded-lg px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-[#F59E0B]"
          value={timeRange}
      ...
      {loading ? (
        <div className="glass-panel rounded-xl p-8 flex items-center justify-center">
          <p className="text-sm text-[#A1A1AA]">Loading analytics...</p>
        </div>
      ) : !stats ? (
        <div className="glass-panel rounded-xl p-8 flex flex-col items-center justify-center gap-2">
          <p className="text-sm text-[#A1A1AA]">No analytics data available</p>
          <p className="text-xs text-[#71717A]">Start using agents to see performance metrics.</p>
        </div>
      ) : (
        <>
          {/* Overview Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Total Sessions</span>
              <div className="text-2xl font-bold text-white mt-1">{stats.totalSessions ?? 0}</div>
              <span className="text-[10px] text-[#71717A]">All time</span>
            </div>
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Total Agents</span>
              <div className="text-2xl font-bold text-white mt-1">{stats.totalAgents ?? 0}</div>
              <span className="text-[10px] text-[#71717A]">{stats.activeAgents ?? 0} active now</span>
            </div>
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Skills Used</span>
              <div className="text-2xl font-bold text-white mt-1">{stats.totalSkills ?? 0}</div>
              <span className="text-[10px] text-[#71717A]">Across all agents</span>
            </div>
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Active Agents</span>
              <div className="text-2xl font-bold text-white mt-1">{stats.activeAgents ?? 0}</div>
              <span className="text-[10px] text-[#71717A]">Currently running</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Success Run Rate</span>
              <div className={`text-2xl font-bold ${statColor(stats.successRate ?? 0, 98.5)} mt-1`}>
                {(stats.successRate ?? 0).toFixed(2)}%
              </div>
              <span className="text-[10px] text-[#71717A]">Target: &gt; 98.5%</span>
            </div>
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Avg Response Latency</span>
              <div className="text-2xl font-bold text-white mt-1">
                {stats.avgLatency ?? 0} ms
              </div>
              <span className="text-[10px] text-[#71717A] font-mono">Across all models</span>
            </div>
            <div className="glass-panel p-5 rounded-xl">
              <span className="text-xs text-[#71717A]">Total Spend</span>
              <div className="text-2xl font-bold text-white mt-1">
                ${(stats.totalSpend ?? 0).toFixed(2)}
              </div>
              <span className="text-[10px] text-[#71717A]">Estimated this period</span>
            </div>
          </div>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 bg-slate-800/50 rounded-full h-2">
                        <div
                          className="h-2 rounded-full bg-gradient-to-r from-[#F59E0B] to-[#F59E0B]"
                          style={{ width: `${Math.min(100, (item.count / maxCount) * 100)}%` }}
                        />
                      </div>
                      <span className="text-xs text-[#A1A1AA] font-mono w-16 text-right">{item.count}</span>
                    </div>
                  );
                })}
              </div>
                ...
                {stats?.dailyActivity?.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={stats.dailyActivity}>
                      ...
                      <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#71717A' }} stroke="rgba(255,255,255,0.06)" />
                      <YAxis tick={{ fontSize: 10, fill: '#71717A' }} stroke="rgba(255,255,255,0.06)" />
                      <Tooltip contentStyle={{ background: '#12121a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.75rem', fontSize: 11 }} />
                      <Area type="monotone" dataKey="count" stroke="#F59E0B" fillOpacity={1} fill="url(#colorRequests)" />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="h-full flex items-center justify-center border border-dashed border-[rgba(255,255,255,0.06)] rounded-lg">
                    <span className="text-xs text-[#71717A]">No data yet — start using agents to see charts</span>
                  </div>
                )}
              </div>
            </div>
                ...
                {stats?.modelUsage?.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats.modelUsage.slice(0, 8)}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
                      <XAxis dataKey="model" tick={{ fontSize: 9, fill: '#71717A' }} stroke="rgba(255,255,255,0.06)" />
                      <YAxis tick={{ fontSize: 10, fill: '#71717A' }} stroke="rgba(255,255,255,0.06)" />
                      <Tooltip contentStyle={{ background: '#12121a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.75rem', fontSize: 11 }} />
                  ...
                  <div className="h-full flex items-center justify-center border border-dashed border-[rgba(255,255,255,0.06)] rounded-lg">
                    <span className="text-xs text-[#71717A]">No data yet</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {stats.recentActivity.map((activity: any, i: number) => (
                  <div key={i} className="flex items-center justify-between py-1.5 border-b border-[rgba(255,255,255,0.06)]/50 last:border-0">
                    ...
                    <span className="text-[10px] text-[#71717A] font-mono">{activity.timestamp}</span>
                  </div>
                ))}
              </div>
            </div>
