...
export function PluginsPage() {
  ...
  function typeBadgeColor(type: string) {
    switch (type?.toLowerCase()) {
      ...
      case "tool": case "developer tool": return "bg-slate-900 text-[#94a3b8] border-[rgba(255,255,255,0.06)]";
      ...
      default: return "bg-slate-900 text-[#475569] border-[rgba(255,255,255,0.06)]";
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto w-full">
      <div className="mb-6">
        ...
        <p className="text-xs text-[#94a3b8] mt-0.5">Extend and customize AI abilities with community and corporate modules.</p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="relative flex-1">
          <input className="w-full px-3 py-1.5 rounded-lg bg-slate-900/50 border border-[rgba(255,255,255,0.06)] text-xs text-white placeholder-slate-500"
            placeholder="Search plugins..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
        <select className="px-3 py-1.5 rounded-lg bg-slate-900/50 border border-[rgba(255,255,255,0.06)] text-xs text-white"
        ...
        <button className="btn-nova px-3 py-1.5 rounded-lg text-xs" onClick={loadPlugins} disabled={loading}>
            ...
            {list.map((plugin) => (
              <div key={plugin.id || plugin.name} className="glass-panel rounded-md p-5 flex flex-col justify-between transition-all hover:border-indigo-500/30">
                <div>
                  ...
                  <p className="text-xs text-[#94a3b8] mb-3 leading-relaxed line-clamp-2">{plugin.description || "No description"}</p>
                ...
                <div className="flex items-center justify-between border-t border-[rgba(255,255,255,0.06)] pt-3 mt-3">
                  <span className="text-[10px] font-mono text-[#475569]">by {plugin.author || "unknown"}</span>
                  <div className="flex gap-1.5">
                    {plugin.installed ? (
                      ...
                      <button className="btn-nova px-3 py-1 rounded text-xs"
          ...
          {list.length === 0 && !loading && (
            <div className="glass-panel rounded-md p-8 flex flex-col items-center justify-center gap-2 mt-4">
              ...
              {(searchQuery || filterType !== "all") ? (
                ...
                <p className="text-xs text-[#475569]">The plugin registry is empty</p>
              )}
            </div>
          )}
        </>
          ...
          <div className="bg-[#0b0f19] border border-[rgba(255,255,255,0.06)] max-w-lg w-full rounded-md overflow-hidden shadow-2xl">
            <div className="bg-[#05080f] px-4 py-3 border-b border-slate-850 flex items-center justify-between">
              <div className="flex items-center gap-2 text-white">
                ...
                <span className="text-[10px] text-[#94a3b8]">Configuration</span>
              </div>
              <button onClick={() => setConfigPlugin(null)} className="text-[#475569] hover:text-white">
            ...
            <div className="p-5 space-y-4">
                    ...
                    <div key={field.key}>
                      ...
                      {field.type === "select" ? (
                        <select value={configValues[field.key] || field.defaultValue || ""}
                          ...
                          className="w-full bg-[#020408]/60 border border-[rgba(255,255,255,0.06)] rounded px-3 py-2 text-xs text-white">
                        ...
                        <input type={field.type === "password" ? "password" : field.type === "url" ? "url" : "text"}
                          ...
                          className="w-full bg-[#020408]/60 border border-[rgba(255,255,255,0.06)] rounded px-3 py-2 text-xs text-white placeholder-slate-600" />
                      )}
                      {field.helpText && <p className="text-[9px] text-[#475569] mt-1">{field.helpText}</p>}
                    </div>
                  ))}
                </>
              ) : (
                <>
                  <p className="text-[11px] text-[#94a3b8]">Enter configuration values for this plugin:</p>
                  {Object.keys(configValues).length === 0 ? (
                    <>
                      <p className="text-[10px] text-[#475569]">No saved config. Add settings below:</p>
                      <div className="flex items-center gap-2">
                        <input type="text" placeholder="Key"
                          className="w-1/3 bg-[#020408]/60 border border-[rgba(255,255,255,0.06)] rounded px-3 py-2 text-xs text-white placeholder-slate-600"
                          value={configValues._newKey || ""}
                          onChange={(e) => setConfigValues(prev => ({ ...prev, _newKey: e.target.value }))} />
                        <input type="text" placeholder="Value"
                          className="flex-1 bg-[#020408]/60 border border-[rgba(255,255,255,0.06)] rounded px-3 py-2 text-xs text-white placeholder-slate-600"
                          value={configValues._newVal || ""}
                          onChange={(e) => setConfigValues(prev => ({ ...prev, _newVal: e.target.value }))} />
                        <button className="btn-nova px-3 py-2 rounded text-[10px]"
                          onClick={() => {
                      ...
                      {Object.entries(configValues).filter(([k]) => !k.startsWith("_")).map(([key, val]) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className="text-[10px] text-[#94a3b8] w-1/3">{key}</span>
                          <input type="text" value={val as string}
                            onChange={(e) => setConfigValues(prev => ({ ...prev, [key]: e.target.value }))}
                            className="flex-1 bg-[#020408]/60 border border-[rgba(255,255,255,0.06)] rounded px-2 py-1 text-[10px] text-white" />
                    ...
                    Object.entries(configValues).map(([key, val]) => (
                      <div key={key}>
                        <label className="text-[11px] text-[#94a3b8] font-medium block mb-1">{key}</label>
                        <input type="text" value={val as string}
                          onChange={(e) => setConfigValues(prev => ({ ...prev, [key]: e.target.value }))}
                          className="w-full bg-[#020408]/60 border border-[rgba(255,255,255,0.06)] rounded px-3 py-2 text-xs text-white font-mono" />
                      </div>
                    ))
                  )}
                </>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-[rgba(255,255,255,0.06)]">
                <button onClick={() => setConfigPlugin(null)}
                  className="px-3 py-1.5 rounded text-xs border border-[rgba(255,255,255,0.06)] text-[#94a3b8] hover:bg-slate-900 transition-all">Cancel</button>
                <button onClick={() => configurePlugin(configPlugin.id || configPlugin.name)}
                  disabled={configSaving}
                  className="btn-nova px-4 py-1.5 rounded text-xs disabled:opacity-40">
