...
function renderMarkdown(text: string): string {
  ...
  let html = text
    ...
    .replace(/^#### (.*$)/gm, '<h4 class="text-sm font-bold text-[#F59E0B] mt-4 mb-1">$1</h4>')
    .replace(/^### (.*$)/gm, '<h3 class="text-base font-bold text-[#F59E0B] mt-4 mb-2">$1</h3>')
    ...
    // Blockquotes
    .replace(/^>\s+(.*)/gm, '<blockquote class="border-l-2 border-[rgba(245,158,11,0.30)] pl-3 text-slate-400 text-[11px] italic my-2">$1</blockquote>')
    // Horizontal rule
...
function AgentReportCard({ memory, onDelete }: { memory: any; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="glass-panel rounded-lg p-5 transition-all border border-[rgba(245,158,11,0.10)] hover:border-[rgba(245,158,11,0.20)]">
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2 min-w-0">
          <span className="w-6 h-6 rounded-full bg-gradient-to-br from-[#F59E0B] to-[#EA580C] flex items-center justify-center text-[10px] font-bold text-black shrink-0">
