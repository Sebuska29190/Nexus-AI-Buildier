...
export function RagPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto w-full" onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
      {dragOver && (<div className="fixed inset-0 z-50 bg-[#6366f1]/5 backdrop-blur-sm flex items-center justify-center pointer-events-none"><div className="bg-[#12121a] border-2 border-dashed border-[#6366f1]/50 rounded-2xl px-8 py-6 text-center shadow-2xl"><FolderOpen size={32} className="mx-auto mb-2 text-[#6366f1]" /><p className="text-[#818cf8] text-lg font-bold mb-1">Drop files or folder here</p><p className="text-[#475569] text-xs">TXT, MD, PDF, CSV, JSON, XML, HTML</p></div></div>)}

      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#6366f1] to-[#8b5cf6] flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.3)]"><BookOpen size={20} className="text-white" /></div>
          <div><h1 className="text-xl font-bold text-white">Knowledge Base (RAG)</h1><p className="text-xs text-[#475569]">Upload documents, ask questions — AI searches your knowledge</p></div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={load} className="glass-input px-3 py-1.5 text-xs flex items-center gap-1.5 hover:bg-[rgba(255,255,255,0.06)] transition-colors cursor-pointer"><RefreshCw size={14} className="text-[#475569]" /> Refresh</button>
          <button onClick={() => folderInput.current?.click()} className="glass-input px-3 py-1.5 text-xs flex items-center gap-1.5 hover:bg-[rgba(255,255,255,0.06)] transition-colors cursor-pointer"><FolderOpen size={14} className="text-[#6366f1]" /> Upload Folder</button>
      ...
      {uploadProgress && (<div className="glass-panel rounded-xl p-4 mb-4 border border-[rgba(255,255,255,0.06)]"><div className="flex items-center justify-between mb-2"><span className="text-xs text-[#94a3b8]">Uploading {uploadProgress.completed}/{uploadProgress.total} files...</span><span className="text-xs text-[#6366f1] font-mono">{Math.round((uploadProgress.completed / uploadProgress.total) * 100)}%</span></div><div className="h-2 rounded-full bg-[rgba(255,255,255,0.04)] overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] transition-all duration-300" style={{ width: `${(uploadProgress.completed / uploadProgress.total) * 100}%` }} /></div>{uploadProgress.current && <p className="text-[10px] text-[#475569] mt-1 truncate">Processing: {uploadProgress.current}</p>}{uploadProgress.errors.length > 0 && <div className="mt-2">{uploadProgress.errors.slice(-3).map((err, i) => <p key={i} className="text-[10px] text-[#ef4444]">✕ {err}</p>)}</div>}</div>)}
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-4"><div className="glass-panel rounded-xl p-4 border border-[rgba(255,255,255,0.06)]">
          <h3 className="text-[10px] text-[#94a3b8] uppercase tracking-wider font-medium mb-3">
          ...
          {loading ? <div className="flex items-center justify-center py-8"><Loader2 size={16} className="text-[#6366f1] animate-spin" /></div>
          : docs.length === 0 ? <div className="text-center py-8"><FileText size={32} className="mx-auto mb-2 text-[#475569]" /><p className="text-[11px] text-[#475569]">No documents yet</p><p className="text-[9px] text-[#475569] mt-1">Upload files or drop a folder</p></div>
          : (() => {
            ...
            return (<div className="space-y-1.5 max-h-[500px] overflow-y-auto">
              {displayed.map((doc) => (<div key={doc.id} onClick={() => viewDoc(doc)} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer border transition-all ${selectedDoc?.id === doc.id ? "bg-[rgba(99,102,241,0.1)] border-[rgba(99,102,241,0.3)]" : "bg-[rgba(255,255,255,0.02)] border-[rgba(255,255,255,0.06)] hover:border-[rgba(255,255,255,0.12)]"}`}>
                ...
                <div className="flex-1 min-w-0"><p className="text-[10px] text-white truncate font-medium">{doc.filename}</p><div className="flex items-center gap-2 text-[8px] text-[#475569]"><span>{(doc.size / 1024).toFixed(0)}KB</span><span>{doc.chunkCount} chunks</span>{doc.status === "processing" && <span className="text-[#f59e0b] animate-pulse">⏳</span>}{doc.status === "ready" && <span className="text-[#22c55e]">✅</span>}{doc.status === "error" && <span className="text-[#ef4444]">❌</span>}</div></div>
                <button onClick={(e) => { e.stopPropagation(); deleteDoc(doc.id); }} className="text-[#475569] hover:text-[#ef4444] text-[9px]">✕</button>
              </div>))}
              {filtered.length < docTotal && (
                <button onClick={async () => { await loadMore(); setDisplayLimit(prev => prev + 50); }} className="w-full py-2 text-[10px] text-[#6366f1] hover:text-[#818cf8] transition-colors">
                  Load more ({docTotal - docs.length} remaining)
                </button>
              )}
              {filtered.length >= docs.length && filtered.length > displayLimit && (
                <button onClick={() => setDisplayLimit(prev => prev + 50)} className="w-full py-2 text-[10px] text-[#6366f1] hover:text-[#818cf8] transition-colors">
        ...
        <div className="col-span-8 space-y-4">
          <div className="glass-panel rounded-xl p-4 border border-[rgba(255,255,255,0.06)]"><h3 className="text-[10px] text-[#94a3b8] uppercase tracking-wider font-medium mb-3">Ask your documents</h3>
            ...
            {answer && <div className="mt-3 bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-lg p-3"><p className="text-[10px] text-[#94a3b8] mb-1">Answer:</p><p className="text-xs text-white whitespace-pre-wrap">{answer}</p></div>}
          </div>
          {selectedDoc && (<div className="glass-panel rounded-xl p-4 border border-[rgba(255,255,255,0.06)]"><div className="flex items-center justify-between mb-2"><h3 className="text-[10px] text-[#94a3b8] uppercase tracking-wider font-medium">📄 {selectedDoc.filename}</h3><span className={`text-[9px] px-1.5 py-0.5 rounded ${selectedDoc.status === "ready" ? "bg-[rgba(34,197,94,0.1)] text-[#22c55e]" : "bg-[rgba(245,158,11,0.1)] text-[#f59e0b]"}`}>{selectedDoc.status}</span></div><div className="max-h-96 overflow-y-auto bg-[rgba(0,0,0,0.3)] rounded-lg p-3"><pre className="text-[10px] text-[#94a3b8] whitespace-pre-wrap font-sans">{docContent.slice(0, 5000)}{docContent.length > 5000 ? "\n\n... (truncated)" : ""}</pre></div></div>)}
        </div>
      </div>
    </div>
  );
