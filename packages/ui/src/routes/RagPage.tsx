      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#00f2fe]/20 to-[#4facfe]/20 border border-[#00f2fe]/30 flex items-center justify-center">
            <svg className="w-4 h-4 text-[#00f2fe]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
          </div>
          <div>
            <h2 className="text-lg font-bold text-white">RAG Knowledge Base</h2>
            <p className="text-[10px] text-slate-500">Upload documents, ask questions — AI searches your knowledge</p>
          </div>
        </div>
        <button onClick={() => fileInput.current?.click()} className="btn-premium px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-4">
          <div className="glass-panel rounded-xl p-4 border border-slate-800/60">
            <h3 className="text-[10px] text-slate-400 uppercase tracking-wider font-medium mb-3">Documents ({docs.length})</h3>
            {loading ? (
                ...
                {docs.map((doc) => (
                  <div key={doc.id}
                    ...
                    className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer border transition-all ${
                      selectedDoc?.id === doc.id ? "bg-[#00f2fe]/10 border-[#00f2fe]/40" : "bg-slate-900/40 border-slate-800/40 hover:border-slate-700"
                    }`}>
        ...
        <div className="col-span-8 space-y-4">
          {/* Query */}
          <div className="glass-panel rounded-xl p-4 border border-slate-800/60">
            <h3 className="text-[10px] text-slate-400 uppercase tracking-wider font-medium mb-3">Ask your documents</h3>
            <div className="flex gap-2">
              <input type="text" value={question} onChange={(e) => setQuestion(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && query()}
                placeholder="Ask anything about your documents..."
                className="flex-1 bg-slate-900/60 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white placeholder-slate-600" />
              <button onClick={query} disabled={querying || !question.trim()}
                className="btn-premium px-4 py-2 rounded-lg text-xs disabled:opacity-40 flex items-center gap-1.5">
            ...
            {answer && (
              <div className="mt-3 bg-slate-900/60 border border-slate-800 rounded-lg p-3">
                <p className="text-[10px] text-slate-400 mb-1">Answer:</p>
          ...
          {selectedDoc && (
            <div className="glass-panel rounded-xl p-4 border border-slate-800/60">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">📄 {selectedDoc.filename}</h3>
