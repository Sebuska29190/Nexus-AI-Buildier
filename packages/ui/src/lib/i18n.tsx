...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [lang, setLangState] = useState(() => {
    try { return localStorage.getItem("nova-lang") || "pl"; } catch { return "pl"; }
  });

  const setLang = useCallback((l: string) => {
    if (dicts[l]) {
      setLangState(l);
      try { localStorage.setItem("nova-lang", l); } catch { /* noop */ }
    }
  }, []);

  const t = useCallback((key: string, fallback = key): string => {
